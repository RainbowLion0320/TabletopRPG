...
describe('scenario progression engine', () => {
  ...
  it('activates only the required finale objective until a route is selected', () => {
    ...
    const negotiation = apply(progress, 'S05', 5, { events: ['EV_CHOOSE_NEGOTIATION'] });
    ...
    expect(negotiation.requestedCheck).toEqual(expect.objectContaining({
  ...
  it('fires the S04 thug event when entry effects cross the 19:00 threshold', () => {
    let progress = createScenarioProgress();
    progress = apply(progress, 'S01', 1, { events: ['EV_ACCEPT_COMMISSION'] }).progress;
    progress = apply(progress, 'S01', 2, { events: ['EV_DISCOVER_I04', 'EV_FIND_I04'] }).progress;
    progress.variables.metMontreal = true;
    progress.worldTime = '1920-07-13T18:45';

    const entered = apply(progress, 'S04', 3, { previousScene: 'S03' });

    expect(entered.progress.worldTime).toBe('1920-07-13T19:10');
    expect(entered.firedEventIds).toEqual(expect.arrayContaining([
      'EV_S04_FOG',
      'EV_S04_THUGS'
    ]));
    expect(entered.narrativeCues).toEqual(expect.arrayContaining([
      expect.stringContaining('暴徒追到贝尔街')
    ]));
    expect(entered.progress.encounters.ENC02.state).toBe('resolved');
  });
  ...
  it('applies an actor-only injury on a combat fumble but not on an ordinary failure', () => {
    let progress = reachFinale();
    progress = apply(progress, 'S05', 5, { events: ['EV_CHOOSE_COMBAT'] }).progress;

    const failure = apply(progress, 'S05', 6, {
      completeTurn: false,
      check: { id: 'CHECK_COMBAT', outcome: 'fail' }
    });
    expect(failure.deltas).toEqual([]);

    const fumble = apply(failure.progress, 'S05', 7, {
      completeTurn: false,
      check: { id: 'CHECK_COMBAT', outcome: 'fumble' }
    });
    expect(fumble.firedEventIds).toContain('EV_COMBAT_FUMBLE');
    expect(fumble.deltas).toEqual([{ field: 'hp', target: 'actor', value: -1 }]);
    expect(fumble.narrativeCues).toContain('攻击严重失误，出手的调查员在甲板混战中受到1点伤害。');
  });
  ...
  it('migrates the previous v8 content hash without losing progress', () => {
    ...
    expect(migrated.moduleVersion).toBe('1.1.19');
    ...
    expect(migrated.migrationLog.at(-1)).toContain('1.0.0 -> 1.1.19');
  });
  ...
  it('migrates the immediately previous scenario content version', () => {
    ...
    expect(migrated.moduleVersion).toBe('1.1.19');
    expect(migrated.beatStates.B02).toBe('active');
    expect(migrated.migrationLog.at(-1)).toContain('1.1.12 -> 1.1.19');
  });
  ...
  it('migrates the 1.1.13 live-play save without losing its current scene', () => {
    ...
    expect(migrated.migrationLog.at(-1)).toContain('1.1.13 -> 1.1.19');
  });
  ...
  it('migrates the 1.1.9 live-play content version without losing progress', () => {
    ...
    expect(migrated.migrationLog.at(-1)).toContain('1.1.9 -> 1.1.19');
  });
  ...
  it('migrates the 1.1.10 live-play content version without replaying S04 entry effects', () => {
    ...
    expect(migrated.migrationLog.at(-1)).toContain('1.1.10 -> 1.1.19');
  });
  ...
  it('migrates saves from the pre-fix 1.1.0 runtime', () => {
    ...
    expect(migrated.moduleVersion).toBe('1.1.19');
    expect(migrated.migrationLog.at(-1)).toContain('1.1.0 -> 1.1.19');
  });
  ...
  it('migrates the saved real-play state from scenario 1.1.1', () => {
    ...
    expect(migrated.moduleVersion).toBe('1.1.19');
    expect(migrated.beatStates.B02).toBe('active');
    expect(migrated.migrationLog.at(-1)).toContain('1.1.1 -> 1.1.19');
  });
  ...
  it('migrates the saved real-play state from scenario 1.1.2', () => {
    ...
    expect(migrated.migrationLog.at(-1)).toContain('1.1.2 -> 1.1.19');
  });
  ...
  it('migrates the saved real-play state from scenario 1.1.4', () => {
    ...
    expect(migrated.migrationLog.at(-1)).toContain('1.1.4 -> 1.1.19');
  });
});
