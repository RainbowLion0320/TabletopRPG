...
## [2026-08-14] fix | 终幕战斗技能、结局叙事与 NPC 资源

- 终幕攻击按角色实际声明的合法武器选择格斗或手枪技能，不再把枪击写死为格斗检定。
- 扩展营救结果语义审核，阻止检定失败时提前叙述埃里克脱困；角色装备改为显式可存档数据。
- 扶桑花号交涉代表替换占位头像，接入正式透明角色立绘；酒保和交涉代表的权威外观同步到实际立绘。

## [2026-08-14] fix | 实局 NPC 外观与叙事标记审核

- 扩充 NPC 体型冲突词，阻止模型把清瘦酒保改写成矮胖形象。
- 过滤普通门窗等装饰性环境短语，仅保留带刮痕、血迹、脚印等证据细节的临时可点击标记。

## [2026-08-18] update | 自由度与同轮多检定结算

- 移除“一轮只保留一个检定”的隐式限制，按玩家和技能建立可持久化检定队列，逐骰结算后统一续写。
- 为作者检定和自由检定增加结果契约；自由成功必须产生局部收益，失败与大失败采用受控的 fail-forward 结果。
- 放宽非权威环境细节、NPC 即时反应和局部关系变化，同时继续禁止它们升级为主线事实或线索。
- 过滤已穷尽目标和刚执行过的重复建议，避免玩家被相同操作困在无进展循环。

## [2026-08-18] update | AI DM 语义复核分层与模板接管移除

- 将 Narrator 语义结果拆分为 `blocking / advisory / warning`：仅权威状态和安全冲突阻断，质量问题只重写一次，普通细节偏差直接放行。
- 删除语义失败后由本地拼装通用调查、失败、切场和终幕文本的路径，玩家可见普通叙事继续由 AI DM 生成。
- Narrator 提示词改为权威状态、连续性事实、氛围细节三层模型，明确允许临时环境、无名路人、NPC 即时反应、局部机会与代价。
- 活动 NPC 作为结构化元数据按权威场景归一化；场景、线索、骰果、HP、遭遇人数、结局和安全边界继续硬校验。
- 同步 SPEC、GDD、AI DM 与提示词 Wiki，并补充语义分级、AI 自主重写和无模板接管回归测试。
