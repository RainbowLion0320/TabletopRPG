...
合理行动不必依赖作者预置事件才能产生价值。非主线检定成功时，AI 可以给出不升级为权威线索的排除结论、位置优势、NPC 即时反应、关系变化、风险降低或新行动方法；失败采用 fail-forward，保留失败事实并落实代价、风险或继续机会。系统会拦截“成功但没有更多信息”的空结算，并生成受控的局部收益兜底。
