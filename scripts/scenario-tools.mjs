...
function moduleDoc(module, hash) {
  ...
  const lines = [
    ...
    '## 线索与失败推进', '',
    ...world.items.map((item) => `- **${item.id} ${item.name}**：${item.appearance} 调查区域词：${item.discovery.searchTerms.join('、')}；检定：${item.discovery.skill}/${item.discovery.difficulty}；失败推进：${item.discovery.failForward}`), '',
