...
export async function runDmTurn(
  ...
  // 1b) P0 优化：System2 与 Narrator 并行执行。
  // System2 的输出（mindModel / prospectiveIntents）不影响 Narrator 调用，
  // 原先串行 await 白白浪费一个 LLM RTT (~2s)。
  // 这里提前启动 Promise，在 Narrator 完成后再 await 结果。
  ...
  const s2Promise = shouldRunSystem2(input.state)
    ? (async () => {
        s2Triggered = true;
        try {
          return await synthesizeSystem2(config, {
            turn: currentTurn,
  ...
  // 3b) P0: 等待 System2 完成（通常在 Narrator 之前就已完成，此处 near-instant）
  const s2Result = await s2Promise;
  if (s2Result) {
    if (s2Result.mindModelUpdates.length) {
      s2MindUpdates = s2Result.mindModelUpdates.map((u) => ({
        npcId: u.npcId,
        partial: {
    ...
    if (s2Result.prospectiveIntents.length) s2Intents = s2Result.prospectiveIntents;
  }
  ...
  // 5b) P1 优化：System1 事实抽取改为延迟执行。
  // 事实抽取只做记忆维护（L2 atomic facts），不影响玩家可见的叙事/事件输出。
  // 改为返回 Promise，让 controller 先 dispatch 主响应、再后台写入 facts。
  const turnNumber = ctx.dynamic.workingMemory.turnCount + 1;
  const narrativeText = resolved.legacyResponse.narrative ?? narrator.narrative;

  const deferredUpdates: DmTurnOutput['deferredUpdates'] = DEFAULT_MEMORY_OPTIONS.enableSystem1
    ? extractFactsFromTurn(config, {
        turn: turnNumber,
        ...
        .then((extracted) => {
          const factsToAppend = extracted.length ? extracted : undefined;
  ...
  if (import.meta.env.DEV) {
    pushTrace({
      ...
      memoryUpdate: memoryUpdate
        ...
        : undefined,
      s1ExtractedFacts: undefined, // P1: facts deferred to background
      s2Synthesized: {
