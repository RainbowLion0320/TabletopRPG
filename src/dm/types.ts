...
export interface DmTurnOutput {
  ...
  /** P1 优化：延迟执行的记忆维护（事实抽取 + 情景记忆），不影响玩家可见输出。
   *  Controller 应先 dispatch 主响应，再 await 此 Promise 写入后台数据。 */
  deferredUpdates?: Promise<DeferredTurnUpdates>;
}

/** 延迟更新的数据子集 */
export interface DeferredTurnUpdates {
  factsToAppend?: import('../types/game').AtomicFact[];
  episodicMemoriesToAdd?: import('../types/game').EpisodicMemoryRecord[];
}
