...
export function inferStoryEventFromActions(
  ...
  const mappings: Array<[string, RegExp]> = [
    ...
    [
      ...
      /选择.{0,8}交涉|和平交涉|愿意.{0,8}交涉|尝试.{0,8}交涉|暂缓攻击|(?:听清|聆听|倾听|理解)[^。；！？\n]{0,16}(?:诉求|条件)|不(?:发动|使用|采取).{0,6}(?:攻击|武力)|(?:放下|放低|收起|垂下).{0,6}(?:武器|枪|警棍)|(?:武器|枪|警棍).{0,6}(?:收起|放下|放低)/
    ],
    [
...
function narrativeHarmsPlayer(narrative: string, playerName: string): boolean {
  const playerNames = [...new Set([playerName, playerName.split('·')[0]])].filter(Boolean);
  const player = playerNames.length === 1
    ? escapeRegex(playerNames[0])
    : `(?:${playerNames.map(escapeRegex).join('|')})`;
  ...
  const adjacentSentencePronounInjury = new RegExp(
    `${player}[\\s\\S]{0,140}(?:他的|其)${bodyPart}[^。；！？\\n]{0,28}${injury}`
  );
  return direct.test(text)
    || struckBodyPart.test(text)
    || pronounInjury.test(text)
    || adjacentSentencePronounInjury.test(text);
}
...
const PLOT_CLAIM_PATTERNS: Array<{ label: string; pattern: RegExp }> = [
  ...
  {
    label: '未授权的目击时间或行踪',
    pattern: /(?:最后一次|上次)[^。；！？\n]{0,28}(?:来|来过|出现|见|见到)[^。；！？\n]{0,40}(?:三天前|10号|十号|周[一二三四五六日天]|星期[一二三四五六日天]|十天前|傍晚)|(?:三天前|10号|十号|周[一二三四五六日天]|星期[一二三四五六日天]|十天前)[^。；！？\n]{0,36}(?:最后一次|来过|出现|见|见到)|(?:老鼠|那人)[^。；！？\n]{0,36}(?:总在|常在|经常在)[^。；！？\n]{0,12}(?:码头|港口)/
  },
  {
    label: '未授权的同行者或新人物',
    pattern: /(?:埃里克|摩勒先生|父亲|他)[^。；！？\n]{0,36}(?:和|与|跟)(?:着)?(?:一个|一名|某个|那个)?[^。；！？\n]{0,20}(?:男人|男子|女人|女子|陌生人|伙计|目击者)[^。；！？\n]{0,24}(?:一起|同行|离开|走|来|去)|(?:和|与|跟)(?:着)?(?:一个|一名|某个|那个)[^。；！？\n]{0,20}(?:男人|男子|女人|女子|陌生人|伙计|目击者)[^。；！？\n]{0,16}(?:一起|同行|离开|走|来|去)/
  },
  {
    label: '犯罪、交易或胁迫关系',
  ...
  {
    label: '线索中的路线或设施细节',
    pattern: /(?:地图|笔记)[\s\S]{0,240}(?:泊位编号|仓库(?:布局|排列|分布)|绕过海关|海关检查站|秘密路线|逃生路线)|(?:第?\s*[一二三四五六七八九十百两\d]+\s*号泊位)/
  },
  {
    label: '未调查地点的活动细节',
